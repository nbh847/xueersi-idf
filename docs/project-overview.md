...
ESP32 工程基于 ESP-IDF 6.1 和 LVGL 9.5。15 页 Dashboard 业务代码仍位于 `main/main.c`；App Framework 运行时（App 描述、Registry、Manager、Navigation、Launcher、全局 Wi-Fi 图标）位于 `main/framework/`；业务 App 位于 `main/apps/`，当前有占位 `Games`（`main/apps/games/`）、`PC Monitor`（`main/apps/pc_monitor/`，节点 11 起显示 Agent Service 快照的真实 PC 指标）、菜单 `Tools`（`main/apps/tools/`）与菜单 `Settings`（`main/apps/settings/`）；System Service 位于 `main/services/`，当前有 `Settings Service`（`main/services/xiaomiao_settings_service.{h,c}`，负责默认值、校验、NVS 读写与安全回退）、`Wi-Fi Service`（`main/services/xiaomiao_wifi_service.{h,c}`，连同私有实现 `xiaomiao_wifi_provisioning.c`、`xiaomiao_wifi_dns.{h,c}` 与 `xiaomiao_wifi_internal.h`）、`Agent Service`（`main/services/xiaomiao_agent_service.{h,c}`，节点 11，连同电脑端 `pc-agent/` Python 后端）和 `Storage Service`（`main/services/xiaomiao_storage_service.{h,c}`，节点 14，拥有 MicroSD 挂载／卸载生命周期）、`Assets Service`（`main/services/xiaomiao_assets_service.{h,c}`，节点 15A，只读挂载 1.5 MB `assets` SPIFFS 分区到 `/assets`，发布 READY／DEGRADED／ERROR 快照，提供 `asset:/`→`/assets` 与 `sd:/`→`/sdcard/xiaomiao` 的逻辑路径解析、拒绝表与 open／read／seek／size／close 及有界目录遍历；不写、不格式化，`sd:/` 只读 Storage Service 快照且从不自动挂载）和 `Font Service`（`main/services/xiaomiao_font_service.{h,c}`，节点 15B，解析私有 XMF1 完整 GB2312 字库（7,445 字形、12／16 px A2，`assets/fonts/xiaomiao-zh-cn.xmf`）：Header／offset／码点递增／CRC32 全量校验，经 Assets Service 读取并校验后整包驻留 PSRAM，导出 12／16 px `lv_font_t`（Montserrat 回退链）与 `xiaomiao_font_small/body/title()` 令牌；任一步失败进入 FALLBACK 并保持英文 UI）。普通固件已接入框架：开机进入 Launcher，按注册顺序显示 `Games`、`PC Monitor`、`Tools`、`Settings` 与 `Hardware Test` 五个入口。全系统 UI 已中文化（节点 15C，已通过实机目视回归）：`main/framework/xiaomiao_i18n.{h,c}` 提供 203 个 `XM_TEXT_*` 文案 ID 与中英双表，`xiaomiao_text(id)` 按 Font Service 状态整体选择中文或英文（默认中文，字库失败整表回退英文，不允许混排半成品）；所有 UI 源文件正文保持纯 ASCII，中文只存在于 i18n 表；串口日志、配网网页、自测、`"Xiaomiao"` 品牌、页码数字与技术缩写（CPU／RAM／GPU／ON／OFF／寄存器值等）有意保留英文。字体统一经 `xiaomiao_fonts.h` 的 `small/body/title` 令牌取用，App 名在注册前运行时解析（`.name = NULL` + init 期赋值）。Launcher 图标走三级回退：`xiaomiao_icons.{h,c}` 从 `asset:/icons/<app-id>.bin`（16 × 16 RGB565、恰好 512 字节）读取并缓存于 PSRAM，任何失败回退现有 LVGL Symbol，再回退占位条；当前 `assets/icons/` 只有占位 README，即实机默认走回退链。当前启动链为：
...
app_main
  ...
  -> Storage Service 初始化 + 首次挂载尝试（失败只记警告，继续启动；A 可在 MicroSD 页重试）
  -> ADC、I2C、LEDC 等硬件初始化
  -> LVGL 显示与 keypad 输入注册
  -> Font Service 初始化（校验 XMF1 字库、建立 PSRAM 索引与缓存；失败只记警告并降级英文）
...
显示屏为 160 × 128 ST7735 兼容面板，使用 SPI2、RGB565 交换字节格式和单块 40,960 字节全屏 DMA 缓冲（内部 RAM）。此前节点 10 曾用两块全屏缓冲；2026-09-25 配网 HTTPD 占用对照后，当前源码为 Wi-Fi 和 HTTPD 释放一块显示缓冲，首个配网会话已实机完成联网和凭据保存，刷新性能待复验。LVGL tick 为 1 ms，UI 目标刷新周期为 16 ms。TFT 与 MicroSD 共用 SPI2，通过独立 CS 分时访问。背光引脚直连 VCC，无法调节亮度，因此 Settings 的 Display 页只陈述该硬件事实。
...
执行 `idf.py -p COM5 flash monitor` 时，`flash` 目标先按需构建 app、bootloader、分区表和资源镜像，构建成功后烧录，再进入串口监视；无需在此前单独运行 `idf.py build`。仅需编译而不烧录时使用 `idf.py build`。
