...
- 状态：已完成（2026-09-20；模型经三次修订，最终版自测固件输出 `LAUNCHER_SELF_TEST: PASS`，普通固件实机行为由人工确认无问题）
...
- 在右列继续按 `→` 翻页，**优先落在下一页同一条行**：从第 1 行右列翻到下一页第 1 行第一格，从第 2 行右列翻到下一页第 2 行第一格。
- 下一页没有对应行时**回退到该页第一行第一格**：5 个 App 时第 2 页只有第 1 行，因此从第 1 页第 2 行右列（Settings）按 `→` 仍然翻页，落在第 2 页第 1 行的 Hardware Test。
- 目标页一个 App 都没有时才不翻页，焦点保持不变。
- 在左列继续按 `←` 反向翻页，同样优先同一行：从第 1 行左列翻回上一页第 1 行第二格，从第 2 行左列翻回上一页第 2 行第二格。上一页只要存在就是满页，因此该格必然存在，不需要回退。
...
2. **翻页键改为 `→`，条件是在右列，目标是下一页同一行第一格，缺行时回退到下一页第一格**：这直接满足“往最右边按才切页、不是往下切页”，并满足“上排翻页到第二页上排、下排翻页到第二页下排第一个”。回退规则满足“不管目标页有没有对应行的 App 都要能切过去，没有就默认去第一排；目标页一个 App 都没有才切不过去”，使 5 个 App 时从第 1 页第 2 行也能进入第 2 页。代价是页内第 2 行的入口（如 Tools、Settings）不在 `→` 的路径上，需要用 `↓` 到达——这正是用户要求保留上下键的原因。
3. **`←` 优先对称规则**：从第 p 页第 r 行左列翻回第 p-1 页第 r 行第二格。上一页只要存在就是满页，因此该格必然存在，不需要回退判断；第 1 页左列按 `←` 保持焦点。
...
- 从 Tools 按 `→` 到 Settings，再按 `→` **翻页并回退到第 2 页第 1 行**的 Hardware Test（第 2 页只有 1 行）。
- 第 2 页没有任何 App 的注册数量下，右列按 `→` 保持焦点不翻页（由自测的数量边界断言覆盖）。
- 按 `↓` 从 Games 到 Tools、从 PC Monitor 到 Settings；`↓` 在最后一行无效、`↑` 在第一行无效，两者都不改变页码。
- A 打开焦点 App、B 返回后焦点与页码保持；五个 App 与 Hardware Test 15 页无回归。

预期结果：四个方向键都有明确作用，翻页只由左右键在最外侧列触发、优先保持同一行并在缺行时回退到下一页第一格，既有打开／返回与生命周期行为不变。
...
> 勾选依据见“验证结果”。自测项有串口日志；普通固件实机项为人工确认，无补充串口日志。

- [x] `←` / `→` 在行内换列：索引 0 ↔ 1、索引 2 ↔ 3。（自测引导路径第 3、12 步；人工确认）
- [x] 在右列按 `→` 优先翻到下一页**同一行**的第一格。（自测引导路径第 4、8 步；人工确认）
- [x] 目标页没有对应行时，翻到该页**第一行第一格**；目标页没有任何 App 时才保持焦点。（数量边界回退断言，`n` 为 5／6）
- [x] 5 个 App 时从第 1 页第 2 行右列（Settings）按 `→` 翻到第 2 页的 Hardware Test。（人工确认）
- [x] 在左列按 `←` 翻回上一页**同一行**的第二格；第 1 页左列按 `←` 保持焦点。（自测引导路径第 1、5、9 步）
- [x] 从第 1 行翻页落在下一页第 1 行，从第 2 行翻页落在下一页第 2 行（后者在目标页有第 2 行时）。（自测引导路径第 4 步 → 索引 4、第 8 步 → 索引 6）
- [x] `↑` / `↓` 在页内换行（0 ↔ 2、1 ↔ 3），在任何情况下都不改变页码。（自测引导路径第 6、7、10、11 步；代码层面上下分支不含页基址与 `PER_PAGE`）
- [x] 第 1 行按 `↑`、最后一行按 `↓` 保持焦点不变。（自测引导路径第 2、7、11 步）
- [x] 每页 4 格时按 `→` 两次即翻页。（自测引导路径第 3→4 步即两次 `→`）
...
- [x] 自测固件输出 `LAUNCHER_SELF_TEST: PASS`，且引导路径与断言已按本次规则改写（数量边界覆盖“目标页只有第 1 行时第 2 行仍能翻过去”的回退断言）。（`guided key path: 13 steps + 3 rounds` + `LAUNCHER_SELF_TEST: PASS`）
- [x] 普通固件五个 App 与 Hardware Test 15 页无回归。（人工确认）
- [x] 设计文档、`docs/project-overview.md`、`ROADMAP.md` 与 `goals/ROADMAP-history.md` 已同步真实状态。
- [x] 人工使用 ESP-IDF 6.1 完成自测构建与普通构建的编译、烧录与实机验证。（自测固件有串口日志；普通固件为人工确认，无补充串口日志）
...
- 从 Tools 按 `→` 到 Settings，再按 `→` **翻页并回退到第 2 页第一行**选中 Hardware Test（第 2 页只有第 1 行）。
- 在任意页的第 1 行按 `↑`、在最后一行按 `↓` 均无反应。
...
预期：四个方向都有独立分支；左右分支的翻页目标由页基址 + `PER_PAGE` + 行号 × `COLUMNS` 组成并带“目标页第一格”的回退，上下分支只使用行号与 `COLUMNS/ROWS`、不含页基址或 `PER_PAGE` 运算（即上下键在代码层面不可能改变页码）。

## 验证结果

### 修订三的自测固件（2026-09-20，人工执行、Agent 复核）-- 通过

人工用 `XIAOMIAO_LAUNCHER_SELF_TEST=ON` 在 `.tmp/build-launcher-lr/` 重新编译烧录后取得串口日志：

- `automatic checks passed, guided key path: 13 steps + 3 rounds` —— 引导路径仍为 13 步（7 个 App 两页都有两行，回退分支由自动断言而非引导路径覆盖）。
- **自动断言包含本次新增的缺行回退检查**：数量边界在 `n` 为 5／6 时断言“从第 1 页第 2 行右列按 `→` 必须落到下一页第一格”；该断言通过（失败会以 `the second row falls back to the next page first cell` 输出 `FAIL` 并 abort）。它覆盖的正是 5 个 App 的实际注册数量。
- 八种数量边界（0／1／2／3／4／5／7／16）与失败路径（`open 'self.app0' failed: ESP_ERR_INVALID_STATE`、两处 `destroy refused: an App is open`）全部通过。
- `guided traversal done`：实机按完 13 步；3 轮 A 打开／B 返回输出 `round 1/3 done` → `round 2/3 done` → `round 3/3 done`，最终唯一标记 `LAUNCHER_SELF_TEST: PASS`。
- 无 panic、无看门狗、无重启循环、无输入失效。

人工只提供了末段日志，未提供完整启动头部；结构性推论（自动断言全过才会推进到引导路径）是判定依据，已如实记录而非补造日志。

### 普通固件（2026-09-20，人工确认）-- 通过

人工在目标板上确认最终模型的实机行为无问题，包含：右列按 `→` 优先翻到下一页同一行、目标页缺行时回退到该页第一格（5 个 App 时从第 1 页第 2 行的 Settings 翻到第 2 页第一行的 Hardware Test）、按 `←` 沿同一行翻回、`↓` 在页内换行且不改变页码、第 1 行按 `↑` 与最后一行按 `↓` 无反应，以及五个 App 与 Hardware Test 15 页无回归。

**证据边界**：本次只有人工口头确认（“现在都没问题了，试过了”），未提供普通固件的补充串口日志，因此不补造 `open`／`closed` 时间戳、`screen children` 数值或焦点索引。

### 已被取代的更早结论

首版“左右线性 ±1”模型（引导路径 16 步）与修订二“缺行时保持焦点”模型（引导路径 13 步）的自测结论均已随模型修订失效，日志见 `goals/ROADMAP-history.md`，不作为当前实现的证据。

## 当前交付与未验证范围

- 已完成：范围、导航模型、边界与失败路径决策、检查点与验收标准；Launcher 源码与自测已按修订三改写（含数量边界的回退断言）；设计文档、`docs/project-overview.md`、`ROADMAP.md` 与 `goals/ROADMAP-history.md` 已同步；静态检查；自测固件的编译、烧录与 13 步实机按键路径；普通固件实机导航的人工确认。
- 证据边界：自测固件有串口日志；普通固件为人工口头确认，无补充串口日志。
- 节点状态：已完成，功能验收无待办。
...
- 2026-09-20（修订二）：用户补充“上排翻页应到第二页上排，下排翻页应到第二页下排的第一个”。修订一的翻页目标固定为下一页第一格，丢失了行信息。规则改为**优先保持同一行**：右列按 `→` 落到下一页同一行第一格，左列按 `←` 落到上一页同一行第二格，目标格不存在时保持焦点。引导路径改为 13 步（7 个 App，两页都有两行）。该版通过自测并取得普通固件人工确认，随后被修订三取代。
- 2026-09-20（修订三，最终）：用户补充“第二排往右也要能切到第二页，应该显示在第一排第一个；不管目标页有没有对应行的 App 都要能切过去，没有就默认去第一排；目标页一个 App 都没有才切不过去”。修订二在目标格不存在时保持焦点，导致 5 个 App 时第 1 页第 2 行无法进入第 2 页。最终规则改为**优先同一行，缺行时回退到目标页第一格，目标页为空才不翻**；数量边界新增该回退的断言。引导路径仍为 13 步（7 个 App 两页都有两行，回退由自动断言覆盖）。
- 验证状态：首版与修订二的自测结论均已作废；**修订三（同行优先、缺行回退）已通过自测固件（13 步，含回退断言）并取得普通固件实机确认**，验收标准全部勾选，详见“验证结果”。
